    ...
    if (array.length === 0) {
        ...
        array.forEach((element) => {
            ...
            <div class="date">
                ...
                <span class="month">${whatMonthIs(element.date.slice(5, 7))}</span>
...
function drawCardDetails(array, container) {
    ...
    [array].forEach((element) => {
        ...
        <img src="${element.image}" class="w-100 h-100 object-fit-cover img-details" alt="people in cinema">
    </div>
    <div class="col-md-8 px-3">
        <div class="card-body px-3 container-text-details">
            <h2 class="card-title text-center text-dark fw-bold">${element.name}</h2>
            <p class="card-text"><small class="text-muted d-flex justify-content-end">${element.date}</small></p>
            ...
            <div class="container">
                ...
                <div class="row">
                    <div class="col-12 d-flex justify-content-center">
                        <p class="card-text fw-bold"><small class="text-success-emphasis">Price: $${element.price}</small>
                        </p>
                    </div>
                </div>
            </div>
...
function drawCheckboxs(arrData, container) {
    ...
    arrData.forEach((element) => {
        ...
        divCheck.innerHTML = `<input class="form-check-input" type="checkbox" id="${element.toString().toLowerCase()}" value=${element}><label
        class="form-check-label" for="${element.toString().toLowerCase()}">${element}</label>`;
